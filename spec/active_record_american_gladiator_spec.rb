...
describe "ActiveRecord American Gladiator" do
  ...
  context "The Eliminator" do
    it "returns all orders placed 2 weeks ago" do
      ...
      # Changeable Start
      orders = Order.where('created_at <= ?', 2.weeks.ago) #using the prebuilt two_weeks_ago doesn't seem to work here and I'm not sure why
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      #   Item.find(id)
      # end
      most_popular_items = Item.joins(:orders).select("items.*, count(items.id) as items_count").group("items.id").order("items_count DESC").limit(2)
      # Changeable Stop
